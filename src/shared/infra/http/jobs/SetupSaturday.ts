...
const SetupSaturday = async (): Promise<void> => {
  ...
  const saturdayProducts = products.filter(product =>
    product.exibition_days.includes(6),
  );

  await saturdayProducts.forEach(async product => {
